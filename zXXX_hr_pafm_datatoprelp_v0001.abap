FUNCTION zXXX_hr_pafm_datatoprelp_v0001.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
*       Generate a dynamic in-table of the fields of our SAP
  LOOP AT lt_tpintsap ASSIGNING FIELD-SYMBOL(<ls_tpintsap>) WHERE infty = iv_infty..
    IF <ls_tpintsap>-fieldname IS NOT INITIAL.
      TRY.
...
*              MESSAGE lv_message TYPE 'E'.
      ENDTRY.
      ls_comp-name = <ls_tpintsap>-fieldname.               "Dynamic in-table fields are given names
...
*Generate internal table via the dynamic table type
    ...
    IF sy-subrc <> 0.
*          MESSAGE e000(zXXX_hr_py_pay0001) WITH TEXT-019.
    ENDIF.
*Generate work area
...
*          MESSAGE e000(zXXX_hr_py_pay0001) WITH TEXT-019.
  ...
  LOOP AT <lt_itab> ASSIGNING <ls_itab>.
    ...
    LOOP AT lt_tpintsap ASSIGNING <ls_tpintsap>.
      ...
      IF sy-subrc = 0.
        IF <ls_tpintsap>-fieldname EQ 'PERNR'. "Even numbers are assigned to odd numbers
          ...
          FROM zXXX_hr_pa_move
            WHERE infty = @iv_infty
              ...
              AND fnsource = @<ls_tpintsap>-fieldname
              AND znumber >= '5551'
              AND znumber <= '7000'
           INTO TABLE @DATA(lt_move).
          SORT lt_move BY znumber.
          DATA: lv_shu TYPE i.
          LOOP AT lt_move ASSIGNING FIELD-SYMBOL(<ls_move>) WHERE fnsource = <ls_tpintsap>-fieldname.
            lv_shu = lv_shu + 1.
            IF  lv_shu MOD 2 = 1.
          ...
          ASSIGN COMPONENT <ls_tpintsap>-fieldname OF STRUCTURE <ls_sapitab> TO FIELD-SYMBOL(<lv_target>).
          IF sy-subrc = 0.
        ...
        ELSE. "If this is not a personnel code
          ASSIGN COMPONENT <ls_tpintsap>-fieldname OF STRUCTURE <ls_sapitab> TO <lv_target>.
          IF sy-subrc = 0.

            IF <lv_source> IS NOT INITIAL. "The source data is not empty, you must go to the configuration table to find it once
              READ TABLE lt_tpintfind INTO DATA(ls_tpint) WITH KEY  fieldname = <ls_tpintsap>-fieldname
